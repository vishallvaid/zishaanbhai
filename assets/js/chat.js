    ...
    sendMessage: async function () {
        ...
        if (text) {
            ...
            try {
                ...
                let userFriendlyError = "Sorry, connection me problem aa rahi hai.";
                if (error.name === 'AbortError') {
                    userFriendlyError = "n8n server response nahi de raha (Timeout). Please check if n8n is listening.";
                } else if (error.message.includes('HTTP_404')) {
                    userFriendlyError = "n8n Test Webhook active nahi hai. Please n8n me 'Execute Workflow' click karein.";
                } else if (window.location.protocol === 'file:') {
                    userFriendlyError = "Browser direct file se connection block kar raha hai (CORS). Please 'Live Server' use karein.";
                }

                this.addMessage(userFriendlyError + " (Error: " + error.message + ")", 'ai');
            }
        }
    },
